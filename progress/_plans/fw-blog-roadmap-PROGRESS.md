...
- 2026-08-10 — 聯絡 Email（`hi@imfw.io`，Cloudflare Email Routing 別名）：`site.json` 新增 `authorEmail`（純地址、不含 `mailto:`），刻意**不**放進 `social`／`authorSameAs`——`sameAs` 的語意是同一實體的其他檔案頁，email 應走 `Person.email`。「關於我」聯絡列加入 secondary 樣式的信封按鈕（GitHub → Email → RSS），JSON-LD 只在 about 頁的 `ProfilePage` 帶 `email`，文章頁的 author 區塊維持不帶，避免地址出現在每一頁被爬蟲收割（build 斷言兩邊都驗；實測全站僅 about/index.html 含該地址，feed/llms.txt/sitemap 皆無）。content 端另加防呆：`authorSameAs` 不得含 `mailto:`、email 不得成為 social 項目。DESIGN.md、docs/README.md 同步。Tests 103/103。
