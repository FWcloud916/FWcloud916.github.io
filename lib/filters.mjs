...
// 概念筆記也由 topics.json 人工策展，與文章 membership 分開保存。
export function topicsForNote(topics = [], noteUrl = "") {
  return topics.filter((topic) => topic.noteUrls?.includes(noteUrl));
}
...
// Garden note 的 related 欄位是明確的 canonical URL；不依賴 tag 或 topic 猜測關係。
export function relatedContent(posts = [], notes = [], currentUrl = "", relatedUrls = []) {
  const itemsByUrl = new Map([...posts, ...notes].map((item) => [item.url, item]));
  return (relatedUrls ?? [])
    .filter((url, index, urls) => url !== currentUrl && urls.indexOf(url) === index)
    .map((url) => itemsByUrl.get(url))
    .filter(Boolean);
}

export function backlinksForTarget(notes = [], targetUrl = "", excludedUrls = []) {
  const excluded = new Set(excludedUrls ?? []);
  return notes
    .filter((note) => (note.data.related ?? []).includes(targetUrl) && !excluded.has(note.url))
    .sort((a, b) => new Date(b.data.updated ?? b.data.created) - new Date(a.data.updated ?? a.data.created));
}
