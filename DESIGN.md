...
- **CTA buttons** (About page contact section, also the existing [404.md](src/404.md) pattern): primary is `bg-blue-600 text-white rounded-lg hover:bg-blue-700`; secondary is `border border-blue-600 text-blue-600 bg-white rounded-lg hover:bg-blue-50`. Both `inline-flex items-center gap-2 px-6 py-3 transition`. The contact row runs GitHub (primary) → email (secondary) → RSS (secondary), so the two ways to reach a person sit together ahead of the subscribe action; the row wraps to a stack below `sm:`.
