...
![Rootful Docker 與 Rootless Docker 的權限邊界比較，顯示 socket、daemon 與 container 在 host root 或非 root user namespace 下的差異。](/assets/images/production-rootless-docker-privilege-model.png)
...
![Rootless Docker 安全部署流程，從只讀 preflight、變更計畫、明確核准、目標使用者安裝到端對端驗證，失敗時停止診斷或回滾。](/assets/images/production-rootless-docker-deployment-workflow.png)
