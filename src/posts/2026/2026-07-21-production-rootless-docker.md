...
5. **End-to-end validation**：檢查 context、socket、process owner、Compose、user service、linger、container 與對外 port。
...
它會檢查 distribution、systemd、target account、`newuidmap`、`newgidmap`、subordinate IDs、rootful units 與 socket、rootless context、user socket、linger、cgroup v2、AppArmor/SELinux、firewall tools 與 low-port policy。
...
### 第六步：處理 80/443 與 firewall

Rootless process 預設不能直接綁定 1024 以下的 privileged ports。最單純的設計是讓 container 發布 8080/8443，再由 host reverse proxy 或外部 load balancer 接 80/443。
...
Rootless networking、privileged ports、host firewall、device access 和嚴格 resource controls 都可能需要額外整合。Docker Desktop、沒有適當 systemd user session 的 WSL、non-systemd 或 immutable hosts，也不適合直接照這篇的 service 流程操作。
...
| 可由 reverse proxy 接 80/443 | Rootless 很適合，container 使用高 host port |
