...
{% image "src/assets/images/podman-rootless-production-engine-privilege-paths.png", "Rootful Docker、Rootless Docker 與 Rootless Podman 的權限路徑比較，顯示 root daemon、user namespace 與 daemonless 本機操作的差異。", "(min-width: 30em) 50vw, 100vw" %}
...
{% image "src/assets/images/podman-rootless-production-production-verification-v2.png", "Rootless Podman 正式環境驗證流程，從 workload 盤點與 host privilege 判斷，依序檢查 UID/GID、網路、cgroup、storage、service lifecycle 與外部驗證。", "(min-width: 30em) 50vw, 100vw" %}
